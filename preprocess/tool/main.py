...
def fetch(root_dir, dataset, filters):
    ...
    for dir_name, label_name in dir_names:
        ...
        feature_vectors_, instance_names_ = load.fetch_dir(dir_name, feature_id_converter, filters)
    ...
    feature_vectors = convert_feature_vectors(feature_vectors, feature_id_converter.unique_num, True)
    instance_names = np.array(instance_names)
    labels = np.array(labels)

    # shuffle
    feature_vectors, instance_names, labels = util.shuffle(feature_vectors, instance_names, labels)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="full",
        choices=("full", "test_ternary_large", "test_binary_large", "test_binary_small"),
    )
    parser.add_argument("--use-ngs-collated-mirna-only", "-u", action="store_true")
    parser.add_argument(
        "--whiten-before-split",
        "-w",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "-n",
        action="store_true",
        help="Normalize the expression levels " "along samples (after splitting).",
    )
    parser.add_argument(
        "--use-important-mirna-only",
        "-I",
        type=int,
        default=-1,
    ...
    parser.add_argument(
        "--remove-unused-mirnas",
        "-D",
        action="store_true",
        help="If true, we remove reported unused miRNAs " "from feature dimensions.",
    )
    ...
    if args.remove_unused_mirnas:
        preprocess_filters.append(preprocess.remove_unused_mirna)
    if args.normalize:
    ...
    if args.use_ngs_collated_mirna_only:
        preprocess_filters.append(preprocess.use_ngs_collated_mirna)
    ...
    if args.use_important_mirna_only > 0:
        postprocess_filters.append(postprocess.create_importance_score_filter(args.use_important_mirna_only))
